...
\begin{tikzpicture}
    ...
    \foreach \i in {0,...,\nodes} {
        ...
        \node[circle, fill=DarkOrchid,draw=DarkOrchid] (r\i) at (\ax, +1.4) {\color{white}$r_\i$};
    ...
    \foreach \i in {0, ..., \nodes} {
        ...
        \draw[-latex,line width=1,color=DarkOrchid] (s\i) -- ($(s\i) + (.6,0)$) to[out=0,in=270]   ($(r\i) - (0,.8)$) -- (r\i);
        \draw[-latex,line width=1,color=DarkOrchid] (s\j) -- ($(s\j) - (.6,0)$) to[out=180,in=270] ($(r\i) - (0,.8)$) -- (r\i);
    ...
    \foreach \i in {0, ..., \nodes} {
        ...
        \node at ($(tmp) - (.2,0)$) {\color{RoyalBlue}$\pi$};
    }

    % value function
    \foreach \i in {1, ..., \nnodes} {
        \node[circle, fill=BlueGreen,draw=BlueGreen] (v\i) at ($(s\i) + (0,1.4)$) {\color{white}$v_\i$};
    ...
    \foreach \i in {0, ..., \nodes} {
        ...
        \draw[-latex,line width=1, color=orange] (a\i) -- ($(a\i) + (0,1.1)$) to[out=90,in=180] ($(s\j) - (.9,0)$) -- (s\j);
        \node[circle, fill=orange,draw=white, inner sep=1.5pt] (tmp) at ($(s\j) - (.8,0)$) {};
        \node at ($(tmp) - (0,.3)$) {\color{orange}$\hat{F}$};
    }
