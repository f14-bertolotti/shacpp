...
A neural network is a function $f_{\theta}:\mathbb{R}^n\rightarrow\mathbb{R}^m$ parameterized by its weights $\theta$, which maps an input vector $x\in\mathbb{R}^n$ to an output vector $y\in\mathbb{R}^m$. 
