...
In this section, we evaluate \fname{} against both PPO and SHAC across various multi-agent scenarios and architectures. An ablation study replacing only the transition function (SHAC+, where the reward function remains differentiable) is provided in \Cref{apx:ablation}.
...
\begin{figure}
    ...
    \caption{Snapshots of the result of the Transport scenario. The agents are represented as colored circles, the package as a red square, and the goal as a green circle. The agents are trained to push the package to the goal.}\label{fig:transport-trained}
    \vspace{0.5cm}
\end{figure}
