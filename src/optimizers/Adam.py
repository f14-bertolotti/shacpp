...
def add_adam_command(base, srcnav=lambda x:x, tgtnav=lambda x:x.agent, attrname = "set_optimizer"):
    ...
    def adam(trainer, lr, betas, weight_decay):
        getattr(srcnav(trainer), attrname)(torch.optim.Adam(tgtnav(trainer).parameters(), lr=lr, betas=betas, weight_decay=weight_decay))
