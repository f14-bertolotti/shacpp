...
def train_reward_model(
    ...
    dataloader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(
            torch.cat([random_obs_data, reward_model_obs_data[reward_model_msk_data]], dim=0),
            torch.cat([random_act_data, reward_model_act_data[reward_model_msk_data]], dim=0),
            torch.cat([random_rew_data, reward_model_rew_data[reward_model_msk_data]], dim=0),
        ),
...
def train():
    ...
    reward_model = RewardModel3(observation_size = observation_size, action_size = action_size, agents = agents, layers = 3, hidden_size = 512, dropout=0.1, activation="ReLU", device = device)
